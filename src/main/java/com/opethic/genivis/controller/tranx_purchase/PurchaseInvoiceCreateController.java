...
public class PurchaseInvoiceCreateController implements Initializable {
    ...
    @FXML
    public Button btnModify, btnCancel;
    ...
    @FXML
    public Label lbTranxDate, lbLedgerName, lbSuplierGSTN, lbPurchaseSerial, lbInvoiceNo, lbInvoiceDate, lbPurchaseAc, lbPaymentMethod, lbPurchaseOrder, lbPurchaseChallan,   purc_orders, purc_challan;
...
//    RadioButton rbCredit, rbCash;
    ...
    private String ledgerStateCode = "", ledgerCode = "";
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        cbPurchaseAc.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<CommonDTO>() {
            @Override
            public void changed(ObservableValue<? extends CommonDTO> observable, CommonDTO oldValue, CommonDTO newValue) {
        ...
        initialEnterMethod();
        cbSupplierGSTIN.setEditable(false);
        cbSupplierGSTIN.setOnKeyPressed(e-> {
...
//            btnModify.setManaged(false);
        ...
        if (purchase_id > 0) {
            ...
            Communicator.purTranxDate = currentDate.format(DateTimeFormatter.ofPattern("dd/MM/yyyy"));
            sceneInitilization();
        }

        tfInvoiceDate.textProperty().addListener((obs, old, nw) -> {
            Communicator.purTranxDate = nw;
        });
...
//        tfInvoiceDateValidation();
    ...
    private void shortcutKeysPurInvoice() {
        purScrollPane.addEventFilter(KeyEvent.KEY_PRESSED, new EventHandler<KeyEvent>() {
                    ...
                    AlertUtility.AlertConfirmation(AlertUtility.alertTypeConfirmation, "Are you sure want to go list ?", input -> {
                        if (input == 1) {
                            if(purchase_id>0){
                                ...
                                PurchaseInvoiceListController.focusStatus = "cancle";
                                GlobalController.getInstance().addTabStatic(PURCHAE_INV_LIST_SLUG, false);
                            }
                        }
                    });
...
//                    validateInvoiceDate();
    ...
    private void setGSTFooterPart() {
        ...
        tc_igst.setCellValueFactory(cellData -> cellData.getValue().sgstProperty());
                ...
                public void handle(WorkerStateEvent workerStateEvent) {
                    ...
                    if (jsonObject.get("responseStatus").getAsInt() == 200) {
                        ...
                        for (JsonElement element : dataArray) {
                            ...
                            String supplierQty = decimalFormat.format(Double.valueOf(item.get("quantity").getAsString()));
    ...
    private void setEditResponse(PurchaseEditResponseDTO purchaseEditResponseDTO) {
        ...
        edit_response_map.put("totalQty", decimalFormat.format(Double.valueOf(purchaseEditResponseDTO.getTotalQty())));
        edit_response_map.put("totalFreeQty", decimalFormat.format(Double.valueOf(purchaseEditResponseDTO.getTotalFreeQty())));
...
//                Communicator.tranxDate =
    ...
    public void handleTfLedgerName() {
        ...
        SingleInputDialogs.openLedgerPopUp(stage, "Creditors", input -> {
            ...
            System.out.println("input ledger selection LEDGER CODE=>" + input[8]);
            tfLedgerName.setText(input[0].toString());
            ledger_id = (String) input[1];
            ledgerStateCode = (String) input[3];
            ledgerCode = (String) input[8];
            Communicator.ledgerCode = ledgerCode;
            purc_orders.setText((String) input[10]);
            purc_challan.setText((String) input[11]);
    ...
    public void setCbPurchaseAc(ActionEvent actionEvent) {
        ...
        System.out.println("purchaseAcID : "+purchaseAcID);
...
//            purchase_discount.setText(String.valueOf(dis_per_float));
    ...
    private void roundOffCalculations() {
        ...
        for (PurchaseInvoiceTable cmpTRowDTO : tvPurchaseInvoiceTable.getItems()) {
            if(!cmpTRowDTO.getNet_amount().isEmpty()) {
    ...
    public void tableInitiliazation() {
        ...
        if (purchase_id > 0) {
            ...
            for (RowListDTO rowListDTO : rowListDTOS) {
                ...
                getSupplierListbyProductId(rowListDTO.getProductId().toString());
                tranxProductDetailsFun(rowListDTO.getProductId().toString());
...
//        Stage stage = (Stage) .getScene().getWindow()
    ...
    public void createPurchaseInvoice() {
        ...
        purchase_invoice_map.put("purchase_id", cbPurchaseAc.getSelectionModel().getSelectedItem().getId());
        ...
        System.out.println("MK : "+purchase_invoice_map);
        ...
        apiClient.setOnSucceeded(new EventHandler<WorkerStateEvent>() {
            ...
            public void handle(WorkerStateEvent workerStateEvent) {
                ...
                if (jsonObject.get("responseStatus").getAsInt() == 200) {
                    ...
                    lblPurInvBarcode.setText(batchJsonObj.get("barcode").getAsString());
...
//                    System.out.println("BatchNO Enter ");
        ...
        if ("tcParticulars".equals(columnName)) {
            ...
            textField.setOnKeyPressed(event -> {
                ...
                if (event.getCode() == KeyCode.ENTER || !event.isShiftDown() && event.getCode() == KeyCode.TAB) {
                    ...
                    System.out.println("size()" + getTableView().getItems().size());
...
//                    getTableView().getItems().addAll(new PurchaseInvoiceTable("", String.valueOf(current_index + 1), String.valueOf(serial_no + 1), "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""));
    ...
    private void openProduct() {
        SingleInputDialogs.openProductPopUpWithLedgerCode(Communicator.stage, "Product", Communicator.ledgerCode, input -> {
        ...
        if (current_table_row.getIs_batch().equals("true") && ledgerid != 0) {
            BatchWindow.batchWindow(String.valueOf(current_index), product_details_to_batch_window, batchWindowTableDTOList, input -> {
                ...
                for (BatchWindowTableDTO batchWindowTableDTOSs : input) {
                    ...
                    System.out.println("total_batch_discount" + total_batch_discount);
                    ...
                    /*String barcode=input.get(0).getBarcode();
                    if(barcode.length()<UserConstants.BARCODE_LENGTH){
                        for(int i=barcode.length();i<UserConstants.BARCODE_LENGTH;i++){
                            barcode=barcode+"0";
                        }
                    }else{
                        barcode=barcode.substring(0,UserConstants.BARCODE_LENGTH);
                    }*/
                    getTableView().getItems().get(Integer.parseInt(input.get(0).getCurrentIndex())).setB_barcode(input.get(0).getBarcode());
...
class ButtonTableCellDelete extends TableCell<PurchaseInvoiceTable, String> {
    ...
    public ButtonTableCellDelete(TableCellCallback<Object[]> DelCallback) {
    ...
    public static void calculateGst(TableView<PurchaseInvoiceTable> tableView, TableCellCallback<Object[]> callback) {
        ...
        for (PurchaseInvoiceTable purchaseInvoiceTable : tableView.getItems()) {
            if (!purchaseInvoiceTable.getQuantity().isEmpty() && Double.parseDouble(purchaseInvoiceTable.getQuantity()) > 0 && !purchaseInvoiceTable.getRate().isEmpty() && Double.parseDouble(purchaseInvoiceTable.getRate()) > 0) {
                ...
                netAmount = Double.parseDouble(purchaseInvoiceTable.getNet_amount().isEmpty()?"0.0":purchaseInvoiceTable.getNet_amount());
                totalNetAmt += netAmount;
                grossAmt = Double.parseDouble(purchaseInvoiceTable.getGross_amount().isEmpty()?"0.0":purchaseInvoiceTable.getGross_amount());
                totalGrossAmt += grossAmt;
                taxableAmt = Double.parseDouble(purchaseInvoiceTable.getTaxable_amt().isEmpty()?"0.0":purchaseInvoiceTable.getTaxable_amt());
                totaltaxableAmt += taxableAmt;
                disAmt = Double.parseDouble(purchaseInvoiceTable.getFinal_dis_amt().isEmpty()?"0.0":purchaseInvoiceTable.getFinal_dis_amt());
                totalDisAmt += disAmt;
                taxAmt = Double.parseDouble(purchaseInvoiceTable.getFinal_tax_amt().isEmpty()?"0.0":purchaseInvoiceTable.getFinal_tax_amt());
                totalTaxAmt += taxAmt;
...
//        calculateGst(tableView, callback);
    ...
    public static void additionalChargesCalculation(String additionalCharges, TableView<PurchaseInvoiceTable> tableView, TableCellCallback<Object[]> callback) {
        ...
        if (!additionalCharges.isEmpty()) {
            ...
            for (int i = 0; i < tableView.getItems().size(); i++) {
                PurchaseInvoiceTable purchaseInvoiceTable = tableView.getItems().get(i);
                totalTaxableAmt = totalTaxableAmt + (purchaseInvoiceTable.getTotal_taxable_amt().isEmpty() ? 0.0 : Double.parseDouble(purchaseInvoiceTable.getTotal_taxable_amt()));
