...
public class PurchaseReturnEditController implements Initializable {
    ...
    @FXML
    private Button btnPurInvLedgerWindowClose, btnPurInvBatchWinClose, btnPurchaseInvAddnlChargWindowClose;
    ...
    private static final Logger purInvCreateLogger = LoggerFactory.getLogger(PurchaseReturnEditController.class);
    private String productId = "";
    private String responseBody;
    private Node[] focusableNodes;
    @FXML
    private AnchorPane PurRtnrootAnchorPane;
    @FXML
    ...
    private Long purRtnInvoiceId = 0L;

    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        //todo : autofocus on TranxDate
        Platform.runLater(() -> tfPurRtnInvDate.requestFocus());
//         Enter traversal
        PurRtnrootAnchorPane.addEventFilter(KeyEvent.KEY_PRESSED, (KeyEvent event) -> {
        ...
        PurRtnrootAnchorPane.sceneProperty().addListener((obs, oldScene, newScene) -> {
            if (newScene != null) {
                Platform.runLater(() -> {
...
//        rowData();
        ...
        tfPurInvLedgName.setOnMouseClicked(actionEvent -> {
            stage = (Stage) PurRtnrootAnchorPane.getScene().getWindow();
...
//                            setPurChallDataToProduct();
    ...
    private void sceneInitilization() {
        PurRtnrootAnchorPane.sceneProperty().addListener((observable, oldScene, newScene) -> {
...
//        map.put("invoiceDate", tfPurRtnInvDate.getText());
    ...
    private void getTranxRtnProductList(Stage stage, String billId) {
        ...
        SingleInputDialogs.openTranxRtnProductList(billId, stage, "Select Products", (input1, input2) -> {
    ...
    public void createPurchaseReturn() {
        ...
        if (gstDetails.size() > 0) {//if Gst details found check the State code of Ledger and Company
            if (gstDetails.get(0).getState().equalsIgnoreCase(ledgerStateCode)) {// if state code of ledger and company is sames then taxFlag=true else taxFlag=false
                taxFlag = true;
            } else {
                taxFlag = false;
            }
...
//        response = APIClient.postFormDataRequest(finalReq, EndPoints.CREATE_PUR_RETURN_ENDPOINT);
        ...
        if (purRtnInvoiceId == 0)
            ...
            apiClient = new APIClient(EndPoints.EDIT_PUR_RETURN_ENDPOINT, finalReq, RequestType.FORM_DATA);
        }
        try {
            apiClient.setOnSucceeded(new EventHandler<WorkerStateEvent>() {
                ...
                public void handle(WorkerStateEvent workerStateEvent) {
                    ...
                    Stage stage = (Stage) PurRtnrootAnchorPane.getScene().getWindow();
                    if (responseBody.get("responseStatus").getAsInt() == 200) {
    ...
    private void getPurRtnById(Long purRtnInvoiceId) {
        PurchaseReturnListController.selectedPrId = 0L;
        Map<String, String> map = new HashMap<>();
        map.put("id", "" + purRtnInvoiceId);
        String formData = Globals.mapToStringforFormData(map);
        APIClient apiClient = new APIClient(EndPoints.GET_BY_ID_PUR_RTN_ENDPOINT, formData, RequestType.FORM_DATA);
        try {
            apiClient.setOnSucceeded(new EventHandler<WorkerStateEvent>() {
                @Override
                public void handle(WorkerStateEvent workerStateEvent) {
                    String response = workerStateEvent.getSource().getValue().toString();
                    PurReturnEditDTO purRtnResponseBody = new Gson().fromJson(response, PurReturnEditDTO.class);
                    if (purRtnResponseBody.getResponseStatus() == 200) {
                        setPurInvoiceFormData(purRtnResponseBody);
                    } else {
                        purInvCreateLogger.debug("Error in response getPurRtnById()");
                    }
                }
            });
            apiClient.setOnCancelled(new EventHandler<WorkerStateEvent>() {
                @Override
                public void handle(WorkerStateEvent workerStateEvent) {
                    purInvCreateLogger.error("Exception in setOnCancelled()--->" +
                            workerStateEvent.getSource().getValue().toString());
                }
            });
            apiClient.setOnFailed(new EventHandler<WorkerStateEvent>() {
                @Override
                public void handle(WorkerStateEvent workerStateEvent) {
                    purInvCreateLogger.error("Exception in setOnFailed()--->" +
                            workerStateEvent.getSource().getValue().toString());
                }
            });
            apiClient.start();
            ...
            e.printStackTrace(new PrintWriter(sw));
            String exceptionAsString = sw.toString();
            purInvCreateLogger.error("Exception in getPurRtnById()--->" + exceptionAsString);
            e.printStackTrace();
        }

    }

    private void setPurInvoiceFormData(PurReturnEditDTO responseBody) {
        ...
        ledgerId = "" + responseBody.getInvoiceData().getSupplierId();
        tfPurInvCreateTranxDate.setText(tranxDate.format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
        tfPurRtnInvDate.setText(invoiceDate.format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
        tfPurRtnInvNum.setText(responseBody.getInvoiceData().getInvoiceNo());
        tfPurRtnsr.setText(String.valueOf(responseBody.getInvoiceData().getPurchaseSrNo()));
        lblPurRtnTotalRowGross.setText(String.valueOf(responseBody.getGrossTotal()));
        lblPurRtnTotalDisc.setText(String.valueOf(responseBody.getTotalPurchaseDiscountAmt()));
        lblPurRtnTotalTax.setText(String.valueOf(responseBody.getTotalTax()));
        tfPurInvLedgName.setText(String.valueOf(responseBody.getInvoiceData().getSupplierName()));
        tfPurRtnDisPer.setText(String.valueOf(responseBody.getDiscountInPer()));
        tfPurInvoiceAddTotalAmt.setText(String.valueOf(responseBody.getInvoiceData().getAdditionalChargesTotal()));
        ...
        lblPurROff.setText(String.valueOf(responseBody.getInvoiceData().getRoundoff()));
        chbPurInvROff.setSelected(responseBody.getInvoiceData().getRoundOffCheck());
        if (responseBody.getInvoiceData().getPaymentMode().equalsIgnoreCase("credit")) {
            rbPaymentModeCredit.setSelected(true);
        } else rbPaymentModeAdjust.setSelected(true);
       /* if (responseBody.getDiscountInAmt() != null) {
        ...
        if (responseBody.getGstDetails() != null) {
            ...
            cmbPurRtnLedgerGst.setItems(gstDetailsList);
            ...
            for (GstDetailsDTO gstDetailsDTO : gstDetailsList) {
                if (gstDetailsDTO.getGstNo().equalsIgnoreCase(gstNo)) {
                    cmbPurRtnLedgerGst.getSelectionModel().select(gstDetailsDTO);
                }
            }
...
//            row.setUnit(mRow.getUnitId());
            ...
            tvPurInvCreateCMPTRow.getItems().add(row);
            index++;
        }
        tvPurInvCreateCMPTRow.refresh();
    }
